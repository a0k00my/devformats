...
const LS_INPUT = 'df-input-python-dataclass';
const LS_SPLIT = 'df-split-python-dataclass';
...
function pyType(value: unknown, fieldName: string, out: string[]): string {
  if (value === null) return 'Optional[Any]';
  if (Array.isArray(value)) {
    if (value.length === 0) return 'List[Any]';
    return `List[${pyType(value[0], fieldName.replace(/s$/, ''), out)}]`;
  }
  if (typeof value === 'object') {
    const className = toPascalCase(fieldName);
    const fields = Object.entries(value as Record<string, unknown>).map(([key, val]) => {
      const fieldType = pyType(val, key, out);
      return `    ${toSnakeCase(key)}: ${fieldType}`;
    });
    out.push(`@dataclass\nclass ${className}:\n${fields.join('\n')}`);
    return className;
  }
...
function jsonToDataclass(json: string, rootName: string): string {
  const data = JSON.parse(json);
  const out: string[] = [];
  pyType(data, rootName, out);
  const header = 'from dataclasses import dataclass\nfrom typing import Any, List, Optional\n\n\n';
  return header + out.reverse().join('\n\n\n');
}

export default function PythonDataclassGenerator() {
  ...
  const doGenerate = useCallback(() => {
    ...
    try { setOutput(jsonToDataclass(input, rootName || 'Root')); setError(''); setErrorLine(null); }
    catch (e: unknown) {
  ...
  const doDownload = () => {
    if (!output) return;
    const url = URL.createObjectURL(new Blob([output], { type: 'text/x-python' }));
    Object.assign(document.createElement('a'), { href: url, download: 'models.py' }).click();
    ...
    <div className="tool-height flex flex-col border-y" style={{ ...toolContainerStyle(isFullscreen), borderColor: 'var(--jfo-border)' }}>
      ...
      <div ref={containerRef} className="flex flex-col md:flex-row flex-1 overflow-hidden" style={{ position: 'relative' }}>
        ...
        <div className="flex flex-col overflow-hidden border-t md:border-l md:border-t-0" style={{ flex: 1, borderColor: 'var(--jfo-border-2)', minWidth: 0 }}>
          <div className="flex items-center justify-between border-b px-3 py-1" style={{ background: 'var(--jfo-panel-hdr)', borderColor: 'var(--jfo-border-2)' }}>
            <span style={{ ...MONO, fontSize: '10px', textTransform: 'uppercase', letterSpacing: '0.08em', color: 'var(--jfo-text-3)' }}>Python Output</span>
          </div>
          <div className="flex-1 overflow-auto p-4" style={{ background: 'var(--jfo-editor)' }}>
            {output ? (
              <pre style={{ ...MONO, lineHeight: '1.65', fontSize: '13px', whiteSpace: 'pre-wrap', wordBreak: 'break-all', color: 'var(--jfo-code)' }} dangerouslySetInnerHTML={{ __html: highlightCode(output, 'python', isLight) }} />
            ) : (
              <div className="flex h-full items-center justify-center text-xs" style={{ ...MONO, color: 'var(--jfo-placeholder)' }}>Python dataclass output appears here</div>
            )}
          </div>
        </div>
      </div>
