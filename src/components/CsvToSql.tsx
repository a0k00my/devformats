...
export default function CsvToSql() {
  ...
  const [table, setTable] = useState(() => (typeof window === 'undefined' ? 'table_name' : localStorage.getItem(LS_TABLE) ?? 'table_name'));
  ...
  useEffect(() => { localStorage.setItem(LS_TABLE, table); }, [table]);

  const doConvert = useCallback((text: string, tableName: string) => {
    if (!text.trim()) { setOutput(''); setError(''); setErrorLine(null); setRowCount(0); return; }
    const result = Papa.parse<Record<string, unknown>>(text.trim(), { header: true, skipEmptyLines: true, dynamicTyping: true });
    if (result.errors.length > 0) {
      const firstErr = result.errors[0];
      setError(`${firstErr.message}${typeof firstErr.row === 'number' ? ` (row ${firstErr.row + 1})` : ''}`);
      setErrorLine(typeof firstErr.row === 'number' ? firstErr.row + 2 : null);
      setOutput(''); setRowCount(0);
      return;
    }
    setOutput(toSqlInserts(result.data, tableName));
  ...
  const doDownload = () => {
    if (!output) return;
    const url = URL.createObjectURL(new Blob([output], { type: 'text/x-sql' }));
    Object.assign(document.createElement('a'), { href: url, download: 'insert.sql' }).click();
  ...
  return (
    <div className="tool-height flex flex-col border-y" style={{ ...toolContainerStyle(isFullscreen), borderColor: 'var(--jfo-border)' }}>
      <div className="toolbar-scroll flex flex-wrap items-center gap-1.5 border-b px-3 py-2" style={{ background: 'var(--jfo-toolbar)', borderColor: 'var(--jfo-border)' }}>
        ...
        <button onClick={handleConvert} className="tb-btn-primary" title="Cmd/Ctrl+Enter">⚡ Convert to SQL</button>
      ...
      <div ref={containerRef} className="flex flex-col md:flex-row flex-1 overflow-hidden" style={{ position: 'relative' }}>
        ...
        <div className="flex flex-col overflow-hidden border-t md:border-l md:border-t-0" style={{ flex: 1, borderColor: 'var(--jfo-border-2)', minWidth: 0 }}>
          ...
          <div className="flex-1 overflow-auto p-4" style={{ background: 'var(--jfo-editor)' }}>
            {output ? (
              <pre style={{ ...MONO, lineHeight: '1.65', fontSize: '13px', whiteSpace: 'pre-wrap', wordBreak: 'break-all', color: 'var(--jfo-code)' }} dangerouslySetInnerHTML={{ __html: highlightCode(output, 'sql', isLight) }} />
            ) : (
              <div className="flex h-full items-center justify-center text-xs" style={{ ...MONO, color: 'var(--jfo-placeholder)' }}>
                {error ? '← fix the error' : 'SQL output appears here'}
              </div>
            )}
          </div>
        </div>
